...
namespace EIV_Common.Encryptions
{
    ...
    {
        public delegate int TransformBlockCall(ReadOnlySpan<byte> data, Span<byte> destination);
        ...
        public byte[] Decrypt(byte[] data)
        {
            ...
            if (maxOutputLength > actual_OutputLength)
                Array.Resize(ref outputData, actual_OutputLength);
            return outputData;
        }
        ...
        private int EncryptBlock(ReadOnlySpan<byte> data, Span<byte> destination) => _encoder.Encrypt(data, destination, _padding);
        private int DecryptBlock(ReadOnlySpan<byte> data, Span<byte> destination) => _encoder.Decrypt(data, destination, _padding);
        ...
        public static int TransformFinal(TransformBlockCall transformBlockCall, ReadOnlySpan<byte> data, Span<byte> destination, int inputBlockSize)
        {
